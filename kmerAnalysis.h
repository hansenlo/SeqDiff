...
void printClusters(std::vector< std::vector<std::string> > &clusterBuffer, google::dense_hash_map<uint_fast32_t, int, stdHash> &clusterFiles, std::vector< std::string > &uniqueReadsBuffer, std::vector<int> &qualityBuffer, std::vector< std::string > &qualityStringBuffer, std::ofstream &uniqueOut, std::vector<std::shared_ptr<std::ofstream> > &files, int tid);
